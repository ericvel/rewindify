...
- **Loop Orange Ink** (`accent-text`): Accent-coloured *text*, measured on all three grounds it can land on: the plate (5.74:1), the well (5.23:1) and the washed span (4.79:1). The word "LOOP" on the switch when armed and `ACTIVE` on the armed saved-loop row. Both name the same active loop state at the two controls that act on it. It was `#AB3D07` — 5.10:1 and 4.64:1 on the first two grounds, and **4.26:1 on the third**, which is the one the saved-loops plate actually prints on, because the wash is *darker* than the plate rather than lighter. The value went a step deeper at the same 20° hue rather than the legend giving up the accent, and every ground gained margin. The lesson is the Floor Rule's: a ground a token has not been measured on is a ground it does not yet clear.
- **Loop Wash** (`accent-wash`): The tint under the armed passage, and the one token in the system whose value is set by a *luminance* budget rather than by taste. An in-loop bar has to clear 3:1 against both this ground and the played bar, which leaves the washed ground roughly 4% of luminance to move in — so the wash buys chroma instead of darkness: a lightened accent laid on thick, landing the ground on `#EFCCBA` at the same 1.13:1 against the well that a raw 11% accent gave, with half again the saturation. 1.13:1 is the ceiling, which is why the wash cannot be the timeline's only signal and the bracket is. The armed saved-loop row reuses it behind its stronger rail and `ACTIVE` legend; no unarmed row takes it.
...
| **Legend** | 10px | 10px | 600, +0.09em, uppercase, Label Ink | The `legend()` mixin. Printed nomenclature: section titles, keyboard labels, "LOOP START", "SPOTIFY PREMIUM REQUIRED", and the header's "Sign out". Its `legend(11px)` step carries three things: the callback screen, the player's boot status, and Sign out at the phone step. |
...
**Desktop (≥900px)** is a fixed-height chassis: `height: 100dvh` with `overflow: hidden`, a 56px header, a body grid of `300px minmax(0, 1fr)`, and a 40px engraved statusbar. The height is fixed, not minimum, on purpose — the play log is as long as the history is, and it is the list that scrolls (`overflow-y: auto` on the sidebar list and on the main column), never the page. The main column uses `justify-content: safe center` so a tall window does not leave dead plate under the controls while a short one keeps its overflow reachable. The rare step-size preference is recessed into the statusbar beside the arrow-key legend; it supports transport without becoming a third practice bay.

**Phone (<640px)** is one full-bleed plate: `width: 100%`, `min-height: 100dvh`, 16px side padding, no maximum. The control group is bottom-anchored (`margin-top: auto` on the transport) so the keys land under the thumb instead of leaving an empty tail. The 48px chassis strip is bled to the plate edges and holds the compact step-size preference opposite the credit, leaving the saved-loop chooser the full practice-control width. Its extra 8px centres the 40px selector with room for the focus ring instead of clipping that ring against the viewport edge. It used to be a 430px-max column centred in the viewport with a hairline down each side; the cap put a 700px tablet — the likeliest thing on a music stand — in a strip with 135px of dead plate either side, and the hairlines went with it, because they marked the column against the plate behind it and there is no longer anything behind it.
...
**Preferences support the task path; they do not enter it.** Step size changes the two outer transport keys, but it is set rarely and its current value is already printed on both. Its selector therefore lives in the chassis strip at both layouts: beside the desktop arrow-key legend, and opposite the mobile credit. The same native select is printed as compact `STEP 5s` in an 82×28px desktop recess and a 92×40px phone recess. The phone form preserves the 40px touch floor; the desktop form matches the density of the keyboard legend it qualifies. Long option names remain available in the native picker without setting the closed control's width.
...
- **Secondary — light cap** (`cap-light()`): a raised plate-toned control for anything that must not read as primary — nudge keys (34/40px square), search buttons (38px), the mobile search close (42px), keycaps, the saved-loops plate's `SAVE`, its full-width `Save current loop` action, and the first-use `SAVE LOOP` trigger. Hover lifts the gradient top to white. Sign out is deliberately *not* on this list; see The Frequency Sets Material Rule.
- **Selector / first-save action — the saved-loops control** (176×52px desktop resting — full width of its line once the controls row stacks it under the nudger — / full-width 62px phone): with stored spans it is a `well()` cut into the plate, printing `SAVED LOOP` in `legend(10px)` over the saved loop the live bounds are sitting on, or `None`, with an upward caret in Label Ink. It is the surface's only chooser field besides the search input, and it is a **recess rather than a cap** because it prints content. With no stored spans there is nothing to choose or print: the same footprint becomes a light `SAVE LOOP` cap and opens directly into the naming form without competing with the dark transport caps. Both states carry `aria-expanded` and `aria-controls`; visible copy supplies the accessible name. `SAVED LOOP` remains singular because the window holds one, and both words stay so it cannot be scanned as the `LOOP` switch a few centimetres away.
- **Selector — step size** (`well()`, 82×28px desktop / 92×40px phone): a compact chassis preference printing `STEP` beside the current quantity and a downward caret. The real native select covers the recess, so the browser still owns pointer and keyboard selection while its verbose `2 seconds` through `15 seconds` options do not widen the closed control. Its visible quantity uses Figure; its label uses Legend. It never borrows a transport cap or competes with the loop bay.
- **Control labels.** A word set *on* a cap is nomenclature, not body copy: 13px/600/+0.06em uppercase, stepping to 12px on desktop. **The register follows the material, not the word** — a word on a well is `legend(10px)` instead, and a control that moves between the two takes the other register with it. The first-save state therefore prints `SAVE LOOP` in the cap register; once it becomes a value-bearing selector, `SAVED LOOP` moves down to `legend(10px)`.
...
- **Chooser plate** — `surface-plate` ground, 4px radius, Popover shadow, 4px padding. Two of them: the search results (320px wide, `max-height: 340px`, drawn *down*) and the saved-loops plate (324px desktop / anchor-width phone, drawn *up*). Both open by being drawn out of their field's slot rather than by appearing; see The Plate Leaves The Slot Rule. The saved-loops plate opens directly into a labelled optional-name field when the track is empty. With rows present it shows a full-width `Save current loop` action only when the live loop is not already an armed saved loop; an armed loop starts the plate directly with its selected row.
- **Menu row** — the saved-loops plate's row, and **the one row in the system with no rule under it**. A chooser plate is a menu, not a printed index: the plate is the container, `surface-raised` on hover or focus marks the row, and a clipped half-row at the bottom of the scroll port says there is more. Name at the left in Row Title, stored range pushed to the right edge in Figure where the tabular digits make their own column, delete mark trailing in Label Ink print. Its ring is inset (`outline-offset: -2px`) because the list scrolls and a ring at the standard offset would be clipped by the scroll port. The armed row reuses the timeline's wash behind its accent rail and prints `ACTIVE` above the stored range, making the reason creation is absent explicit. Rows do not stagger.
...
**The Frequency Sets Material Rule.** A control's material usually states how often it is pressed. Dark caps belong to constant transport or to the whole-flow entry action (`Continue with Spotify`); light caps belong to contextual work (nudge keys, search, and saving a loop); print belongs to things pressed almost never. The first-use `SAVE LOOP` stays light despite being the only available saved-loop action: it activates a secondary feature inside an active practice session and must not compete with transport. The loop switch answers to group membership instead: it is a recess because it joined the nudger, whose A and B rows are recesses. Sign out is the rarest action in the product, and it shipped for a while as a 122px light cap carrying an eject mark — heavier than the brand beside it, and on the phone a matched twin of the search cap, so the header read as two controls of equal standing when one of them is used every session and the other once. It is now a printed `legend()` word with the anchor hairline from `base.scss` under it, and its hit box is bought with `min-height` the rule never sees: `text-decoration` paints under the glyphs, not under the box, so the word stays print while the target clears 44px on the phone. A new header action earns a cap by naming how often it is pressed.
...
The rule also binds a *scope*. Both search fields say "Search Spotify", because `/search?type=track` returns the whole catalogue: the desktop field said "Search your library" for a while, which named a collection this product never queries, and it disagreed with the phone overlay beside it. One search, one sentence, and the count under it is `matches` at both steps rather than `matches` on one and `tracks` on the other.
...
**The Clear Ground Rule.** Nothing the loop has to *print* may rely on ground the waveform occupies. Every warm neutral in this system is within a few percent of every other, so no ground-level device can carry a signal on the field at all: the wash caps at 1.13:1 against the well, a deeper well floor at 1.06:1, and orange over an unplayed bar at 1.07:1. The bars are mirrored about the centre line inside an envelope inset 6px from each field edge. The outer 3px holds the bracket rail and the inner 3px stays bare well, guaranteeing air between the signal and even a full-scale peak; the knob and scale strips remain clear beyond it for the boundary-rule overshoots. A new loop mark on the timeline earns its place by naming the ground it is measured against, and if that ground is a bar, it needs a gutter or a knockout of its own.
...
**The One Authored Moment Rule.** Arming the loop is the only designed motion in the build: the span and both bracket rails sweep open from their centre together (`transform: scaleX(0 → 1)` plus opacity) over `--arm-duration` (320ms) on `--ease-out` (`cubic-bezier(0.16, 1, 0.3, 1)`, exponential, not a linear fade), and the boundary rules and grips fade in 120ms behind them, through `--behind-sweep` (`calc(var(--arm-duration) * 0.375)`) rather than a literal — which is this rule enforced on itself: the delay shipped as `120ms` three times, twice in `TrackTimeline.vue` and once in the gate's `LoopDemo.vue`, survived `reduce`, and arrived as a 120ms stall in front of an otherwise instant state change. Both components declare the derivation on their own root, beside the geometry, because both already keep their strip heights there. The armed row's rail and wash in the saved-loops plate sweep with them, from their own centres on the same tokens — the gesture is the loop's, not the timeline's, so it reaches wherever the loop is stated. Anything the loop adds anywhere joins this sweep rather than getting a moment of its own, which is why applying a saved loop lights the bracket, the nudger chips, the switch label and the row as one event. The region is mounted whether or not the loop is on, so the span *sweeps* rather than *appears*. Everything else is instant, a 110ms cap press, or derived from those two tokens by arithmetic — see the rule below. `prefers-reduced-motion: reduce` zeroes both durations at `:root`, which disables every transition in the system at once. New motion must be expressed through these two tokens, or through a `calc()` on one of them, so it inherits that; a component may name such a derivation locally (`--lift-duration: calc(var(--press-duration) * 2)`) but may not declare a duration of its own, because a literal `220ms` would survive `reduce` and nothing would catch it.
...
- **Do** measure a new text colour against `surface-well`, not the plate — the well is the binding case, and `ink-label` at 5.02:1 is the floor. And measure it against *every* ground it can land on: the washed span is darker than both, and `accent-text` cleared the plate and the well for two rounds while failing on the one surface the saved-loops plate prints it on.
...
- **Do** express new motion through `--arm-duration` and `--press-duration` so `prefers-reduced-motion` keeps working — **delays included**. A literal delay in front of a zeroed duration is a stall with nothing behind it, and it is the one form of this mistake that still looks like motion after `reduce`.
...
- **Don't** let a control's accessible name replace the value it prints. `aria-label` on an `<output>` does exactly that: the position readout was named "Current position" and its figure — the largest object on the surface and the reason the screen exists — was not in the name at all.
...
- **`timeDisplay` (default `Remaining`) is a persisted preference with no control on any surface.** `skipSeconds` now has a recessed `STEP` selector with 2, 5, 10 and 15-second positions; the remaining-versus-total clock preference is still read from `localStorage` without a UI path.
...
- **13px is carrying four unrelated registers.** It is the small step of Body (the alert chips, the desktop search input, the overlay empty state, the saved-loops plate's guidance), the `sm` step of Row Title, the phone step of Figure (the duration beside the position), and the phone step of the **control label** — 600/uppercase/+0.06em, the only one of the four that steps *down* to 12px on desktop while the rest stay at 13px. Each occurrence carries a different weight/tracking/tabular triple (600 uppercase tracked; 600 tight; 500 tabular; 400 plain) and none share a purpose, so this remains **four roles that collided on a number, not one role used four times**. The control-label role now has two clear occupants in the same workflow: the first-use `SAVE LOOP` cap and the form's `SAVE` cap. More important than the count is the rule they confirm: the register belongs to the material, not to the control, and content may not take the cap register at all.
...
- **The position readout no longer speaks.** `<output>` is a status region by
  default, so the figure was being announced every second of playback — unusable,
  and the reason `aria-live` is now `off`. The trade is that a screen-reader user
  gets no spoken confirmation of a step, a nudge or a loop wrap either; the name
  carries the current value, so it can be read on demand, but nothing announces a
  discrete move. Saved loops already have an authored `role="status"` for exactly
  this reason (`SavedLoopsSelect.vue`), and the transport wants the same
  treatment: a message per *act*, not per tick.

- **Nothing prints that saved loops belong to the loaded track while the list has rows.** The drawer's `SAVED ON THIS TRACK` legend went with the drawer's header, deliberately: it was the largest of the nine horizontal lines the round set out to remove, and the field's own `SAVED LOOP` legend has no room for the qualifier at 176px. First use now states the scope directly in its guidance ("Saved loops stay with this track"), so the moment a user first encounters persistence is covered — but a populated plate asserts it only by changing when the track does. Either the window's legend earns the words back at a width the controls row cannot currently spare, or the plate needs a form of the statement that is not a header.
