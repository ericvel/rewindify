...
- **Loop Orange Deep** (`accent-strong`): The same hue pushed to carry white text at 5.12:1, and — because it is also the only accent value that clears 3:1 on the well (3.85:1) — the loop's whole printed figure: the two rails, the two boundary rules, the A and B markers, their matching chips in the nudger, the rail on the armed row in the saved-loops drawer, and the switch track at both steps. One value, one object, at six scales — and the switch sitting on the same value as the chips beside it is not a coincidence to remove: they share a ground, so they share the value that clears on it. The drawer's rail is the fifth and the only one off the timeline; it earns the same value because it states the same thing — this is the loop — about a row rather than about a span.
...
**Wrap, don't squash.** The desktop controls row is `flex-wrap: wrap` with `gap: 16px 22px`, and the saved-loops handle keeps to the right edge of whichever line it lands on. At 900px the three control groups need more room than the column has, and an unwrapped row scrolled the whole column sideways. Folding the loop switch into the nudger took 145px off the row and gave 66px of it back as the cell. Measured as it now stands, the row carries 806px of content — 212px transport, 402px nudger, 148px handle, two 22px gaps — and holds one line down to a 1170px window, wrapping at 1169px and standing in three lines at 900px with a shorter last one. Renaming the handle from `PASSAGES` to `SAVED LOOPS` spent 21px of that budget and moved the threshold 21px out with it, which is the whole cost of the clearer word: it lands on the row's one right-anchored control, so what it buys is one wrap 21px earlier rather than anything squashed.
...
- **Cap Light Held** (`box-shadow: inset 0 2px 4px rgba(88,80,64,0.34)`): The same sustained press on a *light* cap, whose ground is plate rather than ink: the saved-loops handle for as long as its drawer is open. It also drops its ground from the raised gradient to flat `surface-plate`, so the handle stops catching light from above while the caps beside it still do. Both held entries exist for one reason — at momentary-press depth a state that persists does not read as a state at all.
...
- **Secondary — light cap** (`cap-light()`): a raised plate-toned control for anything that must not read as primary — nudge keys (34/40px square), search buttons (38px), the mobile search close (42px), keycaps, and the saved-loops handle. Hover lifts the gradient top to white. Sign out is deliberately *not* on this list; see The Frequency Sets Material Rule.
- **Disclosure — the saved-loops handle** (`cap-light()`, 52px desktop / 62px phone): the only light cap in the system with a *sustained* pressed state, held in the Cap Light Held shadow for as long as its drawer is open. The loop switch used to share its band on the phone and its label register at both steps; the switch has moved into the nudger at both steps now, so this is the only control in its band on the phone and the controls row's one right-anchored control on desktop. That is the honest arrangement — the handle recalls stored spans and the switch arms the live one. It prints its count (`SAVED LOOPS · 3`) because that figure is the only thing telling a shut drawer apart from an empty one. Carries `aria-expanded` and `aria-controls`. Its label is **two words on purpose**: it printed `PASSAGES` first, a word nothing else on the plate printed and so a word with no stated relation to the `LOOP` switch beside it or to the `SAVED ON THIS TRACK` legend in its own drawer — and a one-word `LOOPS` would have stood a few centimetres from that switch in the same uppercase register, differing by a letter. *Passage* stays the word for the music; the handle names what a row is.
- **Control labels.** A word set *on* a cap is nomenclature, not body copy: `SAVED LOOPS` and the drawer's `SAVE` take one register — 13px/600/+0.06em uppercase, stepping to 12px on desktop. Two peers, one setting; a third wording at that size is drift. **The register follows the material, not the word.** `LOOP` was the third peer and has left it at both steps: the switch is a recess now, so its word is `legend(10px)`, the register this system prints on a well. A control that moves from a cap to a recess takes its label's register with it.
...
- **Saved-loops drawer** — `well(4px)`, holding a printed index of the spans saved on the loaded track. Cut *into* the plate below the controls rather than floated over them, which is the whole reason it does not touch The One Popover Rule: the search results stay the only thing in this product that leaves the surface. Its reveal is a single grid row growing `0fr → 1fr` at `--lift-duration` on `--ease-out`, with the gap to the controls held *inside* the reveal so a shut drawer costs no height and no gap. No opacity — the fade belongs to the loop's sweep, and a drawer coming out of a chassis does not dissolve into one. Capped at three and a half rows (four and a half on the wide plate) so the fourth is visibly clipped: a half-row is the honest scroll affordance, and the cap is what keeps the drawer's reflow of the centred desktop column bounded to about 28px. Its rows are Index Rows in everything but the armed one's accent rail, and its rows do not stagger.
...
- **Do** cut a new region into the plate rather than floating it. The saved-loops drawer is a `well(4px)` opened by a grid-row reveal, which is how it gained a whole surface without touching The One Popover Rule.
- **Do** let a control's material state how often it is pressed, and let a *sustained* state sit one step deeper than a momentary press — `--shadow-key-held` on ink, `--shadow-cap-light-held` on plate.
...
- **Don't** print whether the loop is on anywhere but the switch and the bracket. The saved-loops drawer refuses a save while the loop is off and says so *only* through its disabled control and that control's accessible name — a `LOOP OFF` legend in the drawer would be a third home for the same bit.
...
- **13px is carrying four unrelated registers, and one of them has become a role.** It is the small step of Body (the alert chips, the desktop search input, the overlay empty state, the drawer's empty state), the `sm` step of Row Title, the phone step of Figure (the duration beside the position), and the phone step of the **control label** — 600/uppercase/+0.06em, the only one of the four that steps *down* to 12px on desktop while the rest stay at 13px. Each occurrence carries a different weight/tracking/tabular triple (600 uppercase tracked; 600 tight; 500 tabular; 400 plain) and none share a purpose, so this remains **four roles that collided on a number, not one role used four times** — 13px is still the one step where a contributor cannot tell from the number which register they are in. What changed is the evidence for the split, three times over. The control label was one element on the loop switch; then the switch, the saved-loops handle and the drawer's Save all took it; and then the switch left again at both steps, for `legend(10px)`, when it became a recess. Two peers with a shared purpose and a shared pair is thinner evidence for a role than three was — and what the round actually established is that the register belongs to the **material**, not to the control: a word on a cap is 13px/600/+0.06em, a word on a well is `legend(10px)`. That is the rule to name in the Typography table when this is lifted out of the collision.
