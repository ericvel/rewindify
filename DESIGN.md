...
- **Loop Orange Ink** (`accent-text`): Accent-coloured *text* on the plate (5.10:1) and on the well (4.64:1). The word "LOOP" on the switch when armed and `ACTIVE` on the armed saved-loop row. Both name the same active loop state at the two controls that act on it.
